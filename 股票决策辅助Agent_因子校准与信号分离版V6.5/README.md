# 个人投资者股票决策辅助 Agent｜因子校准与信号分离版 V6.5
...
V6.5 针对历史复现中“Agent结论与后续行情差距过大”进行保守校准。这不代表未来预测一定准确，而是减少没有历史证据支持的肯定判断。

- “个人适配”只回答用户能否承受该股票的风险，不再代表股价将上涨。
- “行情方向”只在当前股票、当前持有期的历史验证通过时输出；未通过时明确显示“不判断方向”。
...
- 加仓与仓位上限增加历史验证硬闸门；方向验证未通过时，不给出加仓信号。
...
| `agent_core.py` | V6.5 行情、量化、风险、方向验证、周期、适配、仓位和卖出逻辑 |
...
| `test_v5.py`—`test_v65.py` | 原有功能、因子模块和V6.5校准回归测试 |
...
2. 新增 `test_v65.py`、`因子校准说明_V6.5.md`、`版本更新清单_V6.5.md` 和 `GitHub部署说明_V6.5.md`；
3. 保留其他现有文件；
4. 等待 Streamlit 自动重启，再重新分析任意股票；
5. 检查结果页是否分别显示 **行情方向、方向可信度、个人适配**。
