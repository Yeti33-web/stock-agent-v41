...
from agent_core import normalize_a_code, normalize_hk_code, normalize_us_code, score_investor
...
def questionnaire_page() -> None:
    ...
    total = len(QUESTIONS)
    original_ui.render_brand("独立测试使用V6.5完整个人风险测评")
    render_test_notice()
    if index < total:
...
def result_page() -> None:
    ...
    if st.session_state.historical_result is None:
        with st.status("正在按历史时点运行完整Agent……", expanded=True) as status:
            try:
                ...
                status.write("4/5 调用V6.5评分、风险、方向验证、周期和仓位逻辑")
                ...
                st.error("没有取得足够的历史时点数据，因此没有生成看似精确但不可靠的结果。")
                st.code(str(exc))
                if st.button("返回修改输入"):
                    st.session_state.historical_view = "analysis"
                    st.rerun()
    ...
    if not bundle.history_complete:
        st.warning("该股票在T日以前的可得历史不足五年。数据不足，无法准确判断，结果仅作低置信度参考。")
