...
def test_low_confidence_analog_does_not_change_score() -> None:
    ...
    assert high_result["analog_used"]
...
def test_market_fallback_is_small_and_reasons_are_explicit() -> None:
    ...
    assert twenty_day["analog_used"]
    assert twenty_day["analog_source"] == "市场基准：测试市场基准"
    assert 0 < twenty_day["analog_adjustment"] <= 3.0
    assert "个股样本数未达到最低要求" in twenty_day["analog_status"]
    ...
    assert "可信度31/100低于35/100" in low_confidence_result["analog_status"]
