# 个人投资者股票决策辅助 Agent｜因子透明与历史验证版 V6.4
...
| 不知道应持有多久 | 对多个期限分别评分，再结合用款时间、目标、看盘条件和纪律选择 | 持有／复核周期及各周期评分 |
...
| 看到了结论，却不知道分数如何形成 | V6.4展示每项因子的当前值、贡献分、公式、权重和缺失规则 | 可审计的“因子贡献解释” |
...
| `agent_core.py` | V6.3 原有行情、量化、风险、相似周期、适配、仓位和卖出逻辑 |
| `factor_analysis.py` | V6.4 因子字典、贡献解释和历史滚动验证 |
...
| `test_v5.py`—`test_v64.py` | 原有功能与新增因子模块回归测试 |

## 首次部署

详细步骤见 [GitHub部署说明.md](GitHub部署说明.md)。概要如下：
...
1. 覆盖 `app.py` 和 `README.md`；
2. 新增 `factor_analysis.py`、`因子字典与历史验证_V6.4.md`、`版本更新清单_V6.4.md` 和 `test_v64.py`；
3. 保留其他现有文件；
4. 等待 Streamlit 自动重启，再重新分析任意股票；
5. 检查结果页是否出现 **“因子解释与验证”** 标签。
