...
V7.0.0 在 V6.5.2 核心之上增加独立的“历史相似情景决策层”（不改动原有评分管线）：

- 当前状态 → 检索历史最相似情景 → 分析这些情景之后的真实表现 → 结合最新资讯与市场实际反应、当前行情与技术形态、原有基本面/估值/风险因子交叉验证 → 动态加权 → 形成最终判断与六段式解释。
- 相似度为可计算模型：IQR 稳健标准化 + 组内加权欧氏距离 + 指数衰减；宏观、行业、政策、事件、预期五个维度因历史数据缺口明确标记“暂不可用”，不回填、不伪造，只降低数据质量分。
- 历史证据分 = 相似度 × 可信度 × 数据质量；证据 HIGH 时历史模块权重 50%—65% 且为最大模块，LOW 时自动降级且权重不超过 20%；四模块权重总和恒等于 100%。
- 历史案例统一统计 1/5/20/60/120 日表现（收益、上涨频率、最大涨幅/回撤、峰值时间、路径形态、量能、突破）。
- 新增“事件—市场反应”判定（正向确认、利好钝化、利好兑现、提前计价、风险确认）与红三兵有效性评估。
- 防前视守卫：行情先截断到分析日 T，选择只用 ≤T 数据，选中后才观察结果；测试证明篡改 T 之后数据不改变选择。
- 测试：原有 46 项 + 新增 21 项，共 67 项全部通过。

详细说明见 [V7.0.0升级说明与交付清单.md](V7.0.0升级说明与交付清单.md) 与 [版本更新清单_V7.0.0.md](版本更新清单_V7.0.0.md)。
...
| 文件 | 用途 |
| --- | --- |
| `app.py` | Streamlit 页面、账户流程和结果展示（含“历史情景决策”页签） |
...
| `lookahead_guard.py` | V7.0.0 时间边界与防前视守卫 |
| `historical_context.py` | V7.0.0 七维度情景登记与历史可得特征构造 |
| `historical_similarity.py` | V7.0.0 相似度数学模型与历史案例检索 |
| `historical_outcome.py` | V7.0.0 历史案例 1/5/20/60/120 日表现统计 |
| `evidence_quality.py` | V7.0.0 历史证据分级（相似度×可信度×数据质量） |
| `event_reaction.py` | V7.0.0 事件—市场反应判定 |
| `technical_patterns.py` | V7.0.0 红三兵有效性与趋势位置分析 |
| `dynamic_weight.py` | V7.0.0 四模块动态权重（总和恒=100%） |
| `historical_decision.py` | V7.0.0 决策编排与六段式解释 |
...
| `snapshot_codec.py` | 完整分析快照序列化与恢复（兼容V7.0.0新增字段） |
...
| `test_v7.py` | V7.0.0 决策层 12 类共 21 项新增测试 |
...
| V7.0.0 | 新增历史相似情景决策层：7维度情景登记、可计算相似度、证据分级、动态权重（证据高时历史≥50%）、事件—市场反应、红三兵有效性、防前视守卫与67项测试全通过 |
