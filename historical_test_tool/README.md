# 股票 Agent 完整界面独立历史时点测试工具 V2.2

这个工具用于回答：

> 假设现在是历史日期 T，Agent A V6.5.2 当时会给出什么风险等级、评分、方向可信度、持有周期和结论？
...
结果页面直接复用当前 Agent A V6.5.2 `app.py` 中的原展示函数，不另写一套简化结果。

V2.2会主动忽略正式 `app.py` 中可能残留的旧版历史测试导入，避免新旧测试文件互相调用。
...
4. 只把在 T 日能够安全确认的数据传给Agent A V6.5.2分析逻辑；
...
先从正式Agent A的 `main` 分支建立 `historical-backtest` 分支，再把整个 `historical_test_tool` 文件夹上传到该分支根目录。上传后目录应类似：
...
检查内容包括：非交易日回退、T 后数据阻断、临时日期恢复、用户实际风险画像参与计算，以及V6.5.2结果页面复用。
