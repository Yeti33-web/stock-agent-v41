...
def factor_catalog() -> list[dict[str, str]]:
    ...
    rows.extend(
        [
            ...
            _catalog_row("基本面评分", "净利率（行业标准化）", "有行业基准：净利率÷行业中位数≥1.3:+8；≥1.0:+4；≥0.6:-1；<0.6:-6；净利率<0:-10；无基准回退：≥15%:+8；5%—15%:+3；<0:-10", "最近可得利润和收入＋同行业中位数（A股行业基准CSV，报告期自动探测）", "与同行业比，高于中位数越明显越正面；负值为风险", "+8至-10分", "无行业基准时回退绝对阈值；缺失则本项0分"),
            ...
            _catalog_row("基本面评分", "资产负债率（行业标准化）", "有行业基准：负债率÷行业中位数≤0.6:+6；≤0.9:+2；≤1.3:-1；>1.3:-6；绝对水平≥95%再-4；无基准回退：≥75%:-10；55%—75%:-4；≤35%:+4", "资产和负债＋同行业中位数（银行/地产等天然高负债行业自动适应）", "低于行业中位数越明显越稳健；绝对高杠杆为风险", "+6至-10分", "无行业基准时回退绝对阈值；缺失则本项0分"),
            ...
            _catalog_row("基本面评分", "投入资本回报率ROIC", "≥15%:+6；8%—15%:+3；<0:-10", "A股：净资产收益率÷权益乘数（ROA口径近似）；美股SEC：NOPAT÷（股东权益＋长期负债－现金）；yfinance：returnOnInvestedCapital", "资本回报率越高，盈利质量越正面；高质量作小幅确认，负值仍为风险", "+6至-10分", "缺失则本项0分"),
            _catalog_row("基本面评分", "自由现金流收益率", "≥5%:+6；0—5%:+2；<0:-8", "美股SEC：经营现金流－资本支出，再÷市值；yfinance：freeCashflow÷市值", "自由现金流越充沛越正面，为负偏负面", "+6至-8分", "A股免费数据源未提供资本支出，本项不参与；缺失则本项0分"),
            _catalog_row("基本面评分", "毛利率／营业利润率趋势", "毛利率≥40%:+3；20%—40%:+2；<0:-6；营业利润率≥20%:+2；5%—20%:+1；<0:-5；两者近期改善+3、恶化-3", "A股：BaoStock毛利率（营业利润率无源）；美股SEC：GrossProfit与OperatingIncomeLoss；yfinance：grossMargins与operatingMargins", "盈利质量与趋势越高越正面，转弱偏负面；绝对水平小幅确认，趋势更有区分度", "约+6至-9分", "缺失则本项0分"),
            _catalog_row("基本面评分", "估值历史分位", "当前PE在自身近5年历史分位≤25%:+3；25%—50%:+2；75%—90%:-3；>90%:-6；亏损≤0:-7", "A股：BaoStock历史peTTM日序列；美股／港股暂无同口径历史序列时不参与", "高估分位风险更明确；低分位可能是价值陷阱，仅小幅加分", "+3至-7分", "历史样本<60个或缺失则本项0分"),
            _catalog_row("基本面评分", "PE相对行业中位数", "PE÷行业中位数<0.7:+4；<1.0:+2；<1.5:-1；≥1.5:-4", "个股PE与A股同行业PE中位数（行业基准CSV）", "横向比较对象：明显低于同行偏正面，明显高于同行偏负面", "+4至-4分", "无行业PE基准或PE≤0时本项0分"),
        ]
    )
