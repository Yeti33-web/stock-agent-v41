...
HISTORY_YEARS = 5
MODEL_VERSION = "V6.7.2"
MODEL_RULESET_ID = "fused-point-in-time-fundamental-v2"
...
def _recent_report_quarters(count: int = 10, as_of: date | None = None) -> list[tuple[int, int]]:
...
_INDUSTRY_BENCH_PATH = Path(__file__).resolve().parent / "行业基准_V6.7.csv"
_INDUSTRY_BENCH_CACHE: dict[str, dict[str, Any]] | None = None


def _load_industry_benchmark() -> dict[str, dict[str, Any]]:
    """读取行业基准（净利率／负债率／PE 中位数），供基本面行业标准化使用。

    V6.7.2 新增：净利率与负债率不再只看绝对值，而是与同行业中位数比较；
    PE 除看自身历史分位外，还看相对同行业中位数的位置。文件缺失或格式
    错误时返回空表，评分自动回退绝对阈值，不影响主流程。
    """
    global _INDUSTRY_BENCH_CACHE
    if _INDUSTRY_BENCH_CACHE is not None:
        return _INDUSTRY_BENCH_CACHE
    table: dict[str, dict[str, Any]] = {}
    try:
        with open(_INDUSTRY_BENCH_PATH, "r", encoding="utf-8-sig") as f:
            for row in csv.DictReader(f):
                ind = (row.get("行业") or "").strip()
                if not ind:
                    continue
                table[ind] = {
                    "样本数": int(row.get("样本数") or 0),
                    "净利率中位数": safe_float(row.get("净利率中位数")),
                    "负债率中位数": safe_float(row.get("负债率中位数")),
                    "PE中位数": safe_float(row.get("PE中位数")),
                    "报告期": row.get("报告期") or "",
                }
    except Exception:
        table = {}
    _INDUSTRY_BENCH_CACHE = table
    return table
...
def _score_fundamentals(fields: dict[str, Any]) -> tuple[float | None, list[str], list[str]]:
    ...
    positives: list[str] = []
    risks: list[str] = []
    industry = fields.get("行业") or ""
    industry_bench = _load_industry_benchmark().get(industry) or {}
    ...
    if margin is not None:
        measurable += 1
        # V6.7.2 行业标准化：优先与同行业净利率中位数比较（消除行业固有差异）
        bench_margin = industry_bench.get("净利率中位数")
        if margin < 0:
            score -= 10
            risks.append("净利率为负")
        elif bench_margin is not None and bench_margin > 0:
            margin_ratio = margin / bench_margin
            if margin_ratio >= 1.3:
                score += 8
                positives.append("净利率显著高于行业中位数")
            elif margin_ratio >= 1.0:
                score += 4
                positives.append("净利率高于或接近行业中位数")
            elif margin_ratio >= 0.6:
                score -= 1
            else:
                score -= 6
                risks.append("净利率明显低于行业中位数")
        elif margin >= 0.15:
            score += 8
    ...
    if debt_ratio is not None:
        measurable += 1
        # V6.7.2 行业标准化：优先与同行业负债率中位数比较（银行/地产等行业天然高负债）
        bench_debt = industry_bench.get("负债率中位数")
        if bench_debt is not None and bench_debt > 0:
            debt_ratio_ratio = debt_ratio / bench_debt
            if debt_ratio_ratio <= 0.6:
                score += 6
                positives.append("资产负债率显著低于行业中位数")
            elif debt_ratio_ratio <= 0.9:
                score += 2
            elif debt_ratio_ratio <= 1.3:
                score -= 1
            else:
                score -= 6
                risks.append("资产负债率明显高于行业中位数")
            if debt_ratio >= 0.95:
                score -= 4
                risks.append("资产负债率绝对水平极高，需关注偿债风险")
    ...
    # ---- V6.7.2 新增：PE 考虑比较对象（相对同行业中位数） ----
    # 与"估值历史分位"互补：历史分位看自身纵向，本因子看同行业横向。
    bench_pe = industry_bench.get("PE中位数")
    if pe is not None and pe > 0 and bench_pe is not None and bench_pe > 0:
        measurable += 1
        pe_ratio = pe / bench_pe
        if pe_ratio < 0.7:
            score += 4
            positives.append("市盈率明显低于行业中位数")
        elif pe_ratio < 1.0:
            score += 2
            positives.append("市盈率低于行业中位数")
        elif pe_ratio < 1.5:
            score -= 1
        else:
            score -= 4
            risks.append("市盈率明显高于行业中位数")

    # ---- V6.7 新增基本面因子：投入资本回报率（ROIC） ----
    # 数据来源：A股为ROA口径近似（净资产收益率÷权益乘数），美股SEC为
    # NOPAT÷投入资本，yfinance为returnOnInvestedCapital。缺失则0分不参与。
    # V6.7.1 校准：历史回测显示“优质白马”高分与后续股价可能负相关（价值陷阱），
    # 因此高质量只作小幅正向确认，负值为风险仍保留较强惩罚。
    roic = ratio_fraction(fields.get("投入资本回报率ROIC"))
    if roic is not None:
        measurable += 1
        if roic >= 0.15:
            score += 6
            positives.append("投入资本回报率相对较高")
        elif roic >= 0.08:
            score += 3
        elif roic < 0:
            score -= 10
    ...
    # ---- V6.7 新增基本面因子：自由现金流收益率（FCF／总市值） ----
    # 数据来源：美股SEC为经营现金流减资本支出，yfinance为freeCashflow；
    # A股免费数据源未提供资本支出，无法可靠计算时该字段缺失，不参与评分。
    fcf_yield = ratio_fraction(fields.get("自由现金流收益率"))
    if fcf_yield is not None:
        measurable += 1
        if fcf_yield >= 0.05:
            score += 6
            positives.append("自由现金流收益率相对较高")
        elif fcf_yield >= 0.0:
            score += 2
        else:
            score -= 8
            risks.append("自由现金流为负，需结合经营与投资开支判断")

    # ---- V6.7 新增基本面因子：毛利率 / 营业利润率 与趋势 ----
    gross_margin = ratio_fraction(fields.get("毛利率"))
    if gross_margin is not None:
        measurable += 1
        if gross_margin >= 0.40:
            score += 3
            positives.append("毛利率相对较高")
    ...
    operating_margin = ratio_fraction(fields.get("营业利润率"))
    if operating_margin is not None:
        measurable += 1
        if operating_margin >= 0.20:
            score += 2
            positives.append("营业利润率相对较高")
        elif operating_margin >= 0.05:
            score += 1
        elif operating_margin < 0:
            score -= 5
            risks.append("营业利润率为负")

    # 利润率趋势：取毛利率与营业利润率中方向更明显的信号；改善+3、恶化-3。
    # 趋势是“边际信息”，比绝对水平更有区分度，保留原幅度。
    margin_trend = fields.get("利润率趋势")
    if margin_trend is not None:
        measurable += 1
        if margin_trend > 0:
            score += 3
            positives.append("毛利率或营业利润率近期改善")
        elif margin_trend < 0:
            score -= 3
            risks.append("毛利率或营业利润率近期走弱")

    # ---- V6.7 新增基本面因子：估值历史分位（当前PE在自身历史中的分位） ----
    # V6.7.1 校准：低分位在趋势下行中可能持续（价值陷阱），正向加分下调；
    # 高估分位作为风险惩罚保留，因为高估值下行风险更明确。
    valuation_percentile = ratio_fraction(fields.get("估值历史分位"))
    if valuation_percentile is not None:
        measurable += 1
        if pe is not None and pe <= 0:
            score -= 7
            risks.append("公司处于亏损状态或市盈率不可用")
        elif valuation_percentile <= 0.25:
            score += 3
            positives.append("估值处于自身历史较低分位")
        elif valuation_percentile <= 0.50:
            score += 2
        elif valuation_percentile > 0.90:
            score -= 6
            risks.append("估值处于自身历史较高分位")
...
def _a_share_pe_percentile(bao_code: str, as_of: date | None = None) -> float | None:
    """Return the percentile rank of the latest TTM P/E within the last 5 years.

    BaoStock provides daily peTTM history; the latest value is ranked against its
    own history so valuation is judged relative to the stock itself, not a fixed
    threshold.  Values <=0 (loss-making) are excluded from the ranking.  When an
    ``as_of`` cutoff is given, only history up to that date is used (no look-ahead).
    """
    try:
        today = as_of or date.today()
        start_text = (today - timedelta(days=5 * 366)).isoformat()
...
def fetch_a_fundamentals(code: str, last_price: float, asset_type: str, as_of: date | None = None) -> EvidenceSnapshot:
    """A-share fundamental snapshot.  With ``as_of`` the report period is used only
    when its disclosure date (pubDate) is not later than ``as_of``, and the P/E
    percentile is computed from history up to ``as_of`` only (no look-ahead)."""
    if asset_type != "A股个股":
    ...
    try:
        ...
        if industry_bench:
            notes.append(
                f"行业标准化：净利率／负债率与『{industry}』同行业中位数比较"
                f"（基准报告期{industry_bench.get('报告期') or '未知'}，样本{industry_bench.get('样本数') or 0}只）。"
            )
        latest_period = ""
        for year, quarter in _recent_report_quarters(as_of=as_of):
            profit = _bao_first_row(bs.query_profit_data(code=bao_code, year=year, quarter=quarter))
            if not profit:
                continue
            if as_of is not None:
                disclosed = str(profit.get("pubDate") or "")
                if disclosed and disclosed > as_of.isoformat():
                    continue  # 该报告期在 as_of 之后才披露，跳过（无前视）
            ...
            equity_multiplier = safe_float(balance.get("assetToEquity"))
            roic_approximation = (
                roe / equity_multiplier
                if roe is not None and equity_multiplier not in {None, 0}
                else None
            )
            gross_margin = safe_float(profit.get("gpMargin"))
            previous_gross_margin = safe_float(previous_profit.get("gpMargin"))
            if gross_margin is not None and previous_gross_margin is not None:
                margin_trend = 1 if gross_margin > previous_gross_margin + 0.005 else -1 if gross_margin < previous_gross_margin - 0.005 else 0
            else:
                margin_trend = None
            fields.update(
                {
                    ...
                    "毛利率": gross_margin,
                    "投入资本回报率ROIC": roic_approximation,
                    "利润率趋势": margin_trend,
                    "行业净利率中位数": industry_bench.get("净利率中位数"),
                    "行业负债率中位数": industry_bench.get("负债率中位数"),
                    "行业PE中位数": industry_bench.get("PE中位数"),
                }
            )
            break
        ...
        if latest_period:
            fields["估值历史分位"] = _a_share_pe_percentile(bao_code, as_of=as_of)
            notes.append("自由现金流因子：A股免费数据源未提供资本支出明细，暂不计算FCF，以经营现金流质量代替。")
            notes.append("基本面分反映财务质量与风险，不预测短期股价方向；历史回测显示高质量股票在部分市场风格下可能跑输（价值陷阱），故本模型不把高质量当作上涨信号。")
            if as_of is not None:
                notes.append(f"历史时点口径：仅使用披露时间不晚于{as_of.isoformat()}的报告期与历史估值。")
        else:
...
def fetch_sec_fundamentals(symbol: str, last_price: float | None) -> EvidenceSnapshot:
    ...
    # V6.7 新增：毛利率／营业利润率／ROIC／自由现金流所需字段
    gross_profit_series = _sec_fact_series(facts, ("GrossProfit",), ("USD",))
    operating_income_series = _sec_fact_series(facts, ("OperatingIncomeLoss",), ("USD",))
    capex_series = _sec_fact_series(facts, ("PaymentsToAcquirePropertyPlantAndEquipment", "PaymentsToAcquirePropertyPlantAndEquipmentIntangibleAssets"), ("USD",))
    debt_series = _sec_fact_series(facts, ("LongTermDebtNoncurrent", "LongTermDebt", "LongTermDebtAndCapitalLeaseObligations"), ("USD",))
    cash_series = _sec_fact_series(facts, ("CashAndCashEquivalentsAtCarryingValue", "CashCashEquivalentsAndShortTermInvestments"), ("USD",))
    tax_series = _sec_fact_series(facts, ("IncomeTaxExpenseBenefit",), ("USD",))
    ...
    gross_profit = _last_value(gross_profit_series)
    operating_income = _last_value(operating_income_series)
    capex = _last_value(capex_series)
    debt = _last_value(debt_series)
    cash = _last_value(cash_series)
    tax = _last_value(tax_series)
    ...
    if len(gross_profit_series) >= 2 and len(revenue_series) >= 2:
        prior_gross_profit = safe_float(gross_profit_series[-2].get("val"))
        prior_gross_margin = (
        ...
        if gross_margin is not None and prior_gross_margin is not None:
            margin_trend = 1 if gross_margin > prior_gross_margin + 0.005 else -1 if gross_margin < prior_gross_margin - 0.005 else 0
    elif len(operating_income_series) >= 2 and len(revenue_series) >= 2:
        prior_operating_income = safe_float(operating_income_series[-2].get("val"))
        prior_operating_margin = (
            prior_operating_income / prior_revenue
            if prior_operating_income is not None and prior_revenue not in {None, 0}
            else None
        )
        if operating_margin is not None and prior_operating_margin is not None:
            margin_trend = 1 if operating_margin > prior_operating_margin + 0.005 else -1 if operating_margin < prior_operating_margin - 0.005 else 0

    # ROIC：NOPAT ÷ 投入资本（股东权益＋长期负债－现金）
    invested_capital = (
        equity + debt - cash
        if equity is not None and debt is not None and cash is not None
        else None
    )
    pretax = income + tax if income is not None and tax is not None else None
    effective_tax_rate = (
        tax / pretax
    ...
    # 自由现金流：经营现金流－资本支出；收益率＝FCF÷市值（由EPS反推股份数）
    free_cash_flow = cashflow - capex if cashflow is not None and capex is not None else None
    implied_shares = income / eps if income is not None and eps not in {None, 0} else None
    fcf_yield = (
        free_cash_flow / (last_price * implied_shares)
        if free_cash_flow is not None and last_price is not None and implied_shares not in {None, 0}
        else None
    )
    ...
    notes = [
        ...
        "投入资本回报率为NOPAT÷（股东权益＋长期负债－现金）的简化口径；自由现金流为经营现金流减资本支出。",
    ]
...
def fetch_us_fundamentals(symbol: str, last_price: float | None = None) -> EvidenceSnapshot:
    ...
    try:
        ...
        fields = {
            ...
            "总市值": market_cap,
            "投入资本回报率ROIC": safe_float(info.get("returnOnInvestedCapital")),
            "自由现金流": free_cash_flow,
            "自由现金流收益率": fcf_yield,
            "毛利率": safe_float(info.get("grossMargins")),
            "营业利润率": safe_float(info.get("operatingMargins")),
        }
