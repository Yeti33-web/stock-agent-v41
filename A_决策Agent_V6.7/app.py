...
def format_field(name: str, value) -> str:
    if value is None or (isinstance(value, float) and pd.isna(value)):
        return "未取得"
    if name in {
        "净资产收益率", "净利率", "净利润同比", "营收同比", "资产负债率", "经营现金流／净利润", "债务／权益",
        "投入资本回报率ROIC", "毛利率", "营业利润率", "自由现金流收益率", "估值历史分位",
        "行业净利率中位数", "行业负债率中位数",
    }:
        parsed = float(value)
    ...
    if name == "利润率趋势":
        parsed = int(value)
        return "改善" if parsed > 0 else "走弱" if parsed < 0 else "持平"
    if name in {"市盈率TTM", "市净率", "行业PE中位数"}:
        return f"{float(value):.3f}"
    if name in {"总市值", "自由现金流"}:
        return f"{float(value):,.3f}"
    return str(value)
...
def render_evidence(bundle, analysis) -> None:
    ...
    if fundamental.fields:
        ...
        if fundamental.score is not None:
            st.caption(f"基本面辅助分：{fundamental.score:.0f}/100。净利率、负债率与同行业中位数比较；PE 同时看自身历史分位与同行业横向位置。")
    else:
