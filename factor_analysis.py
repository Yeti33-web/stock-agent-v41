...
def factor_catalog() -> list[dict[str, str]]:
    """Return the auditable factor dictionary used by V6.5.2.
    ...
    rows.extend(
        [
            ...
            _catalog_row("持有期时点评分", "同周期动量", "clip(Rh÷(2×近60日波动×√h),-1,1)×8", "当前与h日前收盘价、近60日日收益波动", "正收益为正面，但需按股票正常波动调整", "原始最多±8分；再乘历史验证系数1／0.70／0.35", "收益或波动不可得时该周期不可用"),
            _catalog_row("持有期时点评分", "相对基准收益", "clip((R股票,h-R基准,h)÷(2×正常波动),-1,1)×7", "股票和对应基准同周期价格", "跑赢基准为正面", "原始最多±7分；再乘历史验证系数1／0.70／0.35", "基准缺失时0分并降低完整度"),
    ...
    rows.extend(
        [
            ...
            _catalog_row("适配与仓位", "时点与有限适配乘数", "先乘历史可靠性1／0.70／0.35；时点<42上限=0；42—54再×0.5；有限适配再×0.5", "所选周期时点评分、历史可靠性和适配结果", "信号可信度较低、偏弱或适配有限时降低仓位", "0—1.0乘数", "时点评分缺失时上限为0"),
        ]
    )
    ...
    rows.extend(
        [
            _catalog_row("加仓条件分", "原时点评分（含可用资讯）", "以所选周期评分为起点；历史验证只降低权重和可信度，资讯仅作有限修正", "当前完整分析和最新公开资讯", "分数越高越支持加仓", "起点0—100；<42硬限制，42—54条件限制", "无可用周期时不允许形成加仓结论"),
...
def timing_contribution_rows(analysis: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    rows: list[dict[str, Any]] = [
        {"模块": "当前时点评分", "因子": "基础分", "当前值": "中性起点", "本次贡献": 50.0, "分值范围": "固定50分", "说明": selected.get("name", "所选周期")},
        {
            ...
            "分值范围": "原始±10分，再乘历史验证系数1／0.70／0.35",
    ...
    rows.append(
        {
            ...
            "分值范围": "原始最多±8分，再乘历史验证系数1／0.70／0.35",
        ...
        [
            {
                ...
                "分值范围": "原始最多±7分，再乘历史验证系数1／0.70／0.35",
            ...
            },
            {
                ...
                "本次贡献": 0.0,
                "分值范围": "技术贡献乘1.0／0.70／0.35",
                "说明": str(validation.get("reason") or "历史验证较弱时降低权重与可信度，但不阻断行情判断"),
            },
        ]
    )
...
def walk_forward_factor_validation(
    ...
    gate_status = str(production_gate.get("status") or "参考性较弱")
    rows.append(
        {
            "key": "production_validation_gate",
            "因子": "组合信号可靠性校准",
            "当前权重": "通过×1；有限通过×0.70；参考性较弱×0.35",
            ...
            "建议": "保留" if gate_status == "通过" else "降低权重",
            "依据": str(production_gate.get("reason") or "历史验证较弱时降低权重与可信度，但不阻断行情判断。"),
        }
    )
...
def build_factor_analysis(
    ...
    return {
        ...
        "policy_note": (
            ...
            "历史验证主要评估能够用日线无前视还原的量价因子。V6.5.2固定采用通过=1、有限通过=0.70、"
            "参考性较弱=0.35的软校准；缺少基本面、宏观、资讯时对应项跳过，不阻断行情判断。"
        ),
    }
